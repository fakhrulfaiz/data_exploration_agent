...
const FinalResponseMessage: React.FC<FinalResponseMessageProps> = ({ 
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const blob = await DataService.exportDataFrame(data.actions.export_dataframe.df_id);
  ...
  const handleDownloadPlots = async () => {
    ...
    try {
      const blob = await DataService.downloadPlots(data.actions.download_plots.plot_urls);
      const filename = data.actions.download_plots.plot_urls.length > 1 ? 'plots.zip' : 'plot.png';
  ...
  return (
    <div className="flex flex-col space-y-4 max-w-none">
      ...
      {(data.actions.export_dataframe?.df_id || data.actions.download_plots?.plot_urls) && (
        <div className="flex flex-wrap gap-2 pt-2 border-t border-border/50">
          ...
          {data.actions.download_plots?.plot_urls && (
            ...
            >
              ...
              {data.actions.download_plots?.plot_urls && data.actions.download_plots.plot_urls.length > 1
