...
class LargePlottingTool(BaseTool):
    ...
    - **Automatic counting and aggregation** - NO need for separate data analysis step
    - Handling categorical data (e.g. bar charts showing counts of YES/NO values)
    - Creating complex visualizations with grouping and statistics
    
    IMPORTANT: This tool can COUNT and AGGREGATE data automatically.
    - For "count how many X", just pass the column name as x_column
    - For "plot distribution of Y", it will automatically count occurrences
    - DO NOT use smart_data_analysis to count/aggregate before plotting - this tool does it for you
    
    Use this tool when:
    - Dataset has more than 100 rows
    - You need to visualize specific columns (e.g. "depicts_sword", "contains_nudity")
    - User requests matplotlib, static image, or high-quality plots
    - You want to plot counts/distributions (it handles aggregation automatically)
    
    Prerequisites:
    - A DataFrame must be available (created by data_exploration_tool)
    
    Parameters:
    - x_column (str): Primary column or category to plot (will be counted automatically if categorical)
    ...
    def _run(
        ...
        *,
        ...
        try:
            ...
            if not data_context or not data_context.df_id:
                return json.dumps({
                    "error": "No active DataFrame found in context. Please run a data retrieval query first.",
                    "error_type": "resource_not_found",
            ...
            if df is None or df.empty:
                return json.dumps({
                    "error": "DataFrame not found along the provided ID or is empty.",
                    "error_type": "resource_not_found",
                    "tool_name": "large_plotting_tool",
                    "details": {"df_id": data_context.df_id},
                    "recoverable": True
        ...
        try:
            ...
            if "Error" in str(result) and "Traceback" in str(result):
                ...
                return json.dumps({
                    "error": f"Error visualizing data: The generated code failed to run. {result}",
                    "error_type": "execution_error",
                    "tool_name": "large_plotting_tool",
                    "details": {"traceback": str(result)},
                    "recoverable": True
                })
        ...
        try:
            ...
            if not os.path.exists(output_filename):
                return json.dumps({
                    "error": f"Error: The plotting code ran but did not create the expected file '{output_filename}'. Output: {result}",
                    "error_type": "execution_error",
                    "tool_name": "large_plotting_tool",
            ...
            # Extract computed statistics from the DataFrame for the plot
            plot_stats = {}
            try:
                # Get value counts for the x_column if it's categorical
                if x_column in df.columns:
                    value_counts = df[x_column].value_counts().to_dict()
                    plot_stats["value_counts"] = {str(k): int(v) for k, v in value_counts.items()}
                    plot_stats["total_rows"] = len(df)
                    plot_stats["x_column"] = x_column
                    if y_column and y_column in df.columns:
                        plot_stats["y_column"] = y_column
            except Exception as stats_error:
                logger.warning(f"Could not extract plot statistics: {stats_error}")
            
            # Return JSON with both plot URL and computed statistics
            return json.dumps({
                "plot_url": public_url,
                "plot_markdown": f"![{title}]({public_url})",
                "plot_type": plot_type,
                "title": title,
                "statistics": plot_stats,
                "description": f"Generated {plot_type} plot with {plot_stats.get('total_rows', len(df))} rows"
            })
            
        except Exception as e:
            logger.error(f"Upload failed: {e}")
            return json.dumps({
                "error": f"Error uploading plot: {str(e)}",
